...
class BluetoothConnect extends StatelessWidget {
  ...
  BluetoothConnect({super.key, required String deviceName}) {
...
class BluetoothConnector {
  ...
  void startScan() async {
    if (!isScanning) {
      for (final connected in await FlutterBlue.instance.connectedDevices) {
        if (kDebugMode) print('Connected: ${connected.name}');
        setDevice(connected);
      }
      FlutterBlue.instance.startScan(timeout: const Duration(seconds: 16));
    }
  }
